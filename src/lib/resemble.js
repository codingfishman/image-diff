...
修改： codingfishman, 为区分视觉稿和页面稿子的不同颜色，对全白颜色做特殊处理，见注释 getTargetErrorColor
*/
...
(function (root, factory) {
  ...
  * codingfishman
  * ============================
  * 很trick的一个方法，对白色进行特殊处理，以区分视觉稿和开发截图
  * 基于一般情况下，页面的默认背景是白色，如果在白色区域上出现了颜色diff，那么即有一方的颜色值不符合规则
  * 此方案有欠缺，只是一个妥协
  * =============================
  * 如果p1是白色，而p2不是，那么视为p2的不同
  * 如果p1不是白色，而p2是白色，那么视为p1的不同
  */
  function getTargetErrorColor (p1, p2) {
    // 如果第二张图全白，视为第一张图的不同
    if (p2.r === p2.g && p2.g === p2.b && p2.r === 255) {
      return errorPixelColor;
    } else {
      return errorPixelTwoColor;
    }
  }

  var errorPixelTransform = {
    flat : function (d1, d2){
      var targetErrorColor = getTargetErrorColor(d1, d2);
      return {
        r: targetErrorColor.red,
        g: targetErrorColor.green,
        b: targetErrorColor.blue,
        a: targetErrorColor.alpha
      };
    },
    movement: function (d1, d2){
      ...
      return {
        r: ((d2.r*(targetErrorColor.red/255)) + targetErrorColor.red)/2,
        g: ((d2.g*(targetErrorColor.green/255)) + targetErrorColor.green)/2,
        b: ((d2.b*(targetErrorColor.blue/255)) + targetErrorColor.blue)/2,
        a: d2.a
      };
    },
    ...
    movementDifferenceIntensity: function (d1, d2){
      ...
      return {
        r: ((1-ratio)*(d2.r*(targetErrorColor.red/255)) + ratio*targetErrorColor.red),
        g: ((1-ratio)*(d2.g*(targetErrorColor.green/255)) + ratio*targetErrorColor.green),
        b: ((1-ratio)*(d2.b*(targetErrorColor.blue/255)) + ratio*targetErrorColor.blue),
        a: d2.a
      };
    }
  };
  ...
  resemble.outputSettings = function(options){
    ...
    if(options.errorTwoColor) {
      for (key in options.errorTwoColor) {
        errorPixelTwoColor[key] = options.errorTwoColor[key] === undefined ? errorPixelTwoColor[key] : options.errorTwoColor[key];
      }
    }
